$(function(){
	...
	class ItemModal {
		...
		updateTarget( el = null ) {
			...
			console.log(this.configEl.find('[target]'));
	...
	menu.delegate('.remove','click', function(){
		$(this).closest('.menu-item').remove();
		storeMenuData();
	});
	...
	function loadMenuData(parsed){
		var data = storage.text();
		if(data == "") return;
		try{
			data = JSON.parse(data);
			for(var i=0; i<data.length; i++){
				...
				var item = data[i];
				if(!item.custom){
					if(item.page in parsed == false) continue;
					item.name = parsed[item.page];
				}
				...
				if('children' in item ){
					for(var n=0; n<item.children.length; n++){
						...
						if(!child.custom){
							if(child.page in parsed == false) continue;
							child.name = parsed[child.page];
						}
						createItem(child, newItem );
					}
				}
			}
		} catch(e) { console.warn('Error while parsing JSON : ' + e.message ) }
